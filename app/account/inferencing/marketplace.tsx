...
const Marketplace = () => {
  ...
  const handleViewImage = (requestId) => {
    if (!address) {
        ...
        <tbody>
          {taskStatus.map((item, index) => (
            <tr key={item.id}>
              ...
              {item.status === 'Done' && (
                <td>
                  <button className="border-[1px] border-white" onClick={() => handleViewImage(item.id)}>View</button>
                  <button className="border-[1px] border-white" onClick={() => handleViewImage(item.id)}>Mint</button>
                </td>
              )}
            </tr>
          ))}
